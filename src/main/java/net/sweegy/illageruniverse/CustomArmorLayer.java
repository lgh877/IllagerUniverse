...
@OnlyIn(Dist.CLIENT)
public class CustomArmorLayer<T extends LivingEntity, M extends EntityModel<T> & ArmorWearingModel, A extends HumanoidModel<T>> extends RenderLayer<T, M> {
	private static final Map<String, ResourceLocation> ARMOR_LOCATION_CACHE = Maps.newHashMap();
	private final A model;
	private final ArmorModelPart modelPart;
	private final EquipmentSlot slot;
	private final boolean innerModel;

	public CustomArmorLayer(RenderLayerParent<T, M> renderer, A model, ArmorModelPart modelPart, boolean innerModel, EquipmentSlot slot) {
		super(renderer);
		this.model = model;
	...
	public void render(PoseStack p_117096_, MultiBufferSource p_117097_, int p_117098_, T p_117099_, float p_117100_, float p_117101_, float p_117102_, float p_117103_, float p_117104_, float p_117105_) {
		p_117096_.pushPose();
		this.getParentModel().translateArmor(modelPart, p_117096_, innerModel);
	...
	protected void setPartVisibility(A p_117126_, ArmorModelPart p_117127_) {
	...
	*/
	...
	/*=================================== FORGE END ===========================================*/
	public enum ArmorModelPart {
		HEAD, BODY, LEFT_ARM, RIGHT_ARM, LEFT_LEG, RIGHT_LEG
	}

	@SuppressWarnings({"rawtypes", "unchecked"})
	public static void addCustomArmorLayers(LivingEntityRenderer renderer, EntityRendererProvider.Context context, ModelLayerLocation innerArmourModelLayerLocation, ModelLayerLocation outerArmourModelLayerLocation) {
		renderer.addLayer(new CustomArmorLayer(renderer, new HumanoidModel(context.bakeLayer(innerArmourModelLayerLocation)), ArmorModelPart.BODY, true, EquipmentSlot.LEGS));
		renderer.addLayer(new CustomArmorLayer(renderer, new HumanoidModel(context.bakeLayer(innerArmourModelLayerLocation)), ArmorModelPart.RIGHT_LEG, true, EquipmentSlot.LEGS));
		renderer.addLayer(new CustomArmorLayer(renderer, new HumanoidModel(context.bakeLayer(innerArmourModelLayerLocation)), ArmorModelPart.LEFT_LEG, true, EquipmentSlot.LEGS));
		renderer.addLayer(new CustomArmorLayer(renderer, new HumanoidModel(context.bakeLayer(outerArmourModelLayerLocation)), ArmorModelPart.HEAD, false, EquipmentSlot.HEAD));
		renderer.addLayer(new CustomArmorLayer(renderer, new HumanoidModel(context.bakeLayer(outerArmourModelLayerLocation)), ArmorModelPart.BODY, false, EquipmentSlot.CHEST));
		renderer.addLayer(new CustomArmorLayer(renderer, new HumanoidModel(context.bakeLayer(outerArmourModelLayerLocation)), ArmorModelPart.RIGHT_ARM, false, EquipmentSlot.CHEST));
		renderer.addLayer(new CustomArmorLayer(renderer, new HumanoidModel(context.bakeLayer(outerArmourModelLayerLocation)), ArmorModelPart.LEFT_ARM, false, EquipmentSlot.CHEST));
		renderer.addLayer(new CustomArmorLayer(renderer, new HumanoidModel(context.bakeLayer(outerArmourModelLayerLocation)), ArmorModelPart.RIGHT_LEG, false, EquipmentSlot.FEET));
		renderer.addLayer(new CustomArmorLayer(renderer, new HumanoidModel(context.bakeLayer(outerArmourModelLayerLocation)), ArmorModelPart.LEFT_LEG, false, EquipmentSlot.FEET));
	}
}